...
class LogMessage(TypedDict):
    level: plugin_proto.LogLevel
    message: str


_LOG_LEVEL_MAP: dict[int, plugin_proto.LogLevel] = {
    logging.DEBUG: plugin_proto.LOG_LEVEL_DEBUG,
    logging.INFO: plugin_proto.LOG_LEVEL_INFO,
    logging.WARNING: plugin_proto.LOG_LEVEL_WARN,
    logging.ERROR: plugin_proto.LOG_LEVEL_ERROR,
    logging.CRITICAL: plugin_proto.LOG_LEVEL_ERROR,
}
...
class RemoteLogHandler(logging.Handler):
    ...
    def format_to_msg(self, record: logging.LogRecord) -> LogMessage:
        ...
        level = _LOG_LEVEL_MAP.get(record.levelno, plugin_proto.LOG_LEVEL_UNSPECIFIED)
    ...
    async def read_loop(
        self,
        output: asyncio.Queue[plugin_proto.RuntimeMessage | None],
        make_msg_func: Callable[[LogMessage], plugin_proto.RuntimeMessage],
    ) -> None:
        ...
        try:
            ...
            logger.debug("remote plugin log reader loop canceled")
            raise
