...
### 4.1 连接池与 Session 粘连 (Pool & Affinity)
...
4.  **配置存储不一致 (Config Sync Risk)**：
    目前的 `WorkflowConfigManager` (见 `internal/engine/manager.go`) 是基于 **本地文件** 实现的。在分布式模式下，若通过网关更新了 Workflow 配置，该变更仅存在于转发到的那个 Pod 节点上。
    *   **风险**：多 Pod 节点之间的配置不一致。
    *   **改进方案**：后续需要引入基于 Redis 或统一数据库的共享配置实现。
5.  **缓冲区堆积**：
